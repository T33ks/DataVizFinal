...
export default function ErrorDisplay({ title = "ERROR", message }: ErrorDisplayProps) {
  return (
    <div className="flex flex-col items-center justify-center p-6 text-center">
      ...
      <h3 className="text-sm font-pixel neon-text-red mb-2">{title}</h3>
      <p className="text-xs text-red-400">{message}</p>
      <div className="mt-4 grid grid-cols-1 gap-2 w-full max-w-xs">
        <div className="pixel-panel border-red-400">
          <p className="text-xs text-red-400">SYSTEM STATUS: ERROR</p>
        </div>
        <div className="pixel-panel">
          <p className="text-xs">RETRY RECOMMENDED</p>
        </div>
      </div>
    </div>
  )
